...
{
  .con0 = { .bLength = sizeof(USB_CONFIGURATION_DESCRIPTOR), .bDescriptorType = USB_CONFIGURATION_DESCRIPTOR_TYPE, .wTotalLength = sizeof(struct t_pcan_description), .bNumInterfaces = 1, .bConfigurationValue = 1, .iConfiguration = 0, .bmAttributes = USB_CONFIG_BUS_POWERED, .MaxPower = 100 },
  .if0 = { .bLength = sizeof(USB_INTERFACE_DESCRIPTOR), .bDescriptorType = USB_INTERFACE_DESCRIPTOR_TYPE, .bInterfaceNumber = 0, .bAlternateSetting = 0, .bNumEndpoints = 4, .bInterfaceClass = 0, .bInterfaceSubClass = 0, .bInterfaceProtocol = 0, .iInterface = 0 },
  .ep1 = { .bLength = sizeof(USB_ENDPOINT_DESCRIPTOR), .bDescriptorType = USB_ENDPOINT_DESCRIPTOR_TYPE, .bEndpointAddress = PCAN_USB_EP_CMDIN, .bmAttributes = USB_ENDPOINT_TYPE_BULK, .wMaxPacketSize = 16, .bInterval = 0 },
  .ep2 = { .bLength = sizeof(USB_ENDPOINT_DESCRIPTOR), .bDescriptorType = USB_ENDPOINT_DESCRIPTOR_TYPE, .bEndpointAddress = PCAN_USB_EP_CMDOUT, .bmAttributes = USB_ENDPOINT_TYPE_BULK, .wMaxPacketSize = 16, .bInterval = 0 },
  .ep3 = { .bLength = sizeof(USB_ENDPOINT_DESCRIPTOR), .bDescriptorType = USB_ENDPOINT_DESCRIPTOR_TYPE, .bEndpointAddress = PCAN_USB_EP_MSGIN, .bmAttributes = USB_ENDPOINT_TYPE_BULK, .wMaxPacketSize = 64, .bInterval = 0 },
  .ep4 = { .bLength = sizeof(USB_ENDPOINT_DESCRIPTOR), .bDescriptorType = USB_ENDPOINT_DESCRIPTOR_TYPE, .bEndpointAddress = PCAN_USB_EP_MSGOUT, .bmAttributes = USB_ENDPOINT_TYPE_BULK, .wMaxPacketSize = 64, .bInterval = 0 },
};
...
static uint8_t device_init( USBD_HandleTypeDef *pdev, uint8_t cfgidx )
{
  ...
  for( int i = 0; i < pcan_usb_dev.if0.bNumEndpoints; i++ )
  {
    ...
    if( ( ep_addr & 0x80 ) != 0 ) pdev->ep_in[ep_addr & EP_ADDR_MSK].is_used = 1;
    else pdev->ep_out[ep_addr & EP_ADDR_MSK].is_used = 1;
  }
...
static uint8_t device_deinit( USBD_HandleTypeDef *pdev, uint8_t cfgidx )
{
  ...
  for( int i = 0; i < pcan_usb_dev.if0.bNumEndpoints; i++ )
  {
    ...
    if( ( ep_addr & 0x80 ) != 0 ) pdev->ep_in[ep_addr & EP_ADDR_MSK].is_used = 0;
    else pdev->ep_out[ep_addr & EP_ADDR_MSK].is_used = 0;
  }
...
uint16_t pcan_usb_send_command_buffer( const void *p, uint16_t size )
{
  ...
  if( pdev->ep_in[PCAN_USB_EP_CMDIN & 0xFU].total_length ) return 0;
  pdev->ep_in[PCAN_USB_EP_CMDIN & 0xFU].total_length = size;
  if( USBD_LL_Transmit( pdev, PCAN_USB_EP_CMDIN, (void*)p, size ) == USBD_OK ) return size;
  return 0;
}
...
{
  USBD_HandleTypeDef *pdev = &hUsbDeviceFS;
  if( pdev->ep_in[PCAN_USB_EP_MSGIN & 0xFU].total_length ) return 0;
  pdev->ep_in[PCAN_USB_EP_MSGIN & 0xFU].total_length = size;
  if( USBD_LL_Transmit( pdev, PCAN_USB_EP_MSGIN, (void*)p, size ) == USBD_OK ) return size;
  return 0;
}
...
{
#if !defined(STM32G431xx)
  /* F042: USB interrupt is polled in main loop */
  HAL_PCD_IRQHandler( &hpcd_USB_FS );
#endif
  /* G431: USB interrupts handled via USB_LP_IRQHandler */
}
...
static uint8_t device_ep0_rx_ready( USBD_HandleTypeDef *pdev ) { UNUSED(pdev); return USBD_OK; }

static uint8_t device_data_in( USBD_HandleTypeDef *pdev, uint8_t epnum )
{
  struct t_class_data *p_data = (void*)pdev->pClassData;
  if( pdev->pClassData == 0 ) return USBD_FAIL;
...
static uint8_t *device_get_hs_cfg( uint16_t *length ) { *length = sizeof(struct t_pcan_description); return (void*)&pcan_usb_dev; }
static uint8_t *device_get_fs_cfg( uint16_t *length ) { *length = sizeof(struct t_pcan_description); return (void*)&pcan_usb_dev; }
static uint8_t *device_get_other_speed_cfg( uint16_t *length ) { *length = sizeof(struct t_pcan_description); return (void*)&pcan_usb_dev; }
static uint8_t *device_get_device_qualifier( uint16_t *length ) { *length = sizeof(USB_DEVICE_QUALIFIER_DESCRIPTOR); return (void*)&dev_qua; }
...
{
  .Init = device_init, .DeInit = device_deinit, .Setup = device_setup,
  .EP0_TxSent = 0, .EP0_RxReady = device_ep0_rx_ready,
  .DataIn = device_data_in, .DataOut = device_data_out,
  .SOF = 0, .IsoINIncomplete = 0, .IsoOUTIncomplete = 0,
  .GetHSConfigDescriptor = device_get_hs_cfg, .GetFSConfigDescriptor = device_get_fs_cfg,
...
{
  if( USBD_Init( &hUsbDeviceFS, &FS_Desc, DEVICE_FS ) != USBD_OK ) { assert( 0 ); }
  if( USBD_RegisterClass( &hUsbDeviceFS, &usbd_pcan ) != USBD_OK ) { assert( 0 ); }
  if( USBD_Start(&hUsbDeviceFS) != USBD_OK ) { assert( 0 ); }
}
...
int pcan_flush_data( struct t_m2h_fsm *pfsm, void *src, int size )
{
  ...
  {
    case 1:
      if( p_data->ep_tx_in_use[pfsm->ep_addr&0x0F] ) { p_data->ep_tx_data_pending[pfsm->ep_addr&0x0F] = size; return 0; }
      pfsm->state = 0;
      /* fall through */
    case 0:
      assert( p_data->ep_tx_in_use[pfsm->ep_addr&0x0F] == 0 );
      if( size > pfsm->dbsize ) break;
